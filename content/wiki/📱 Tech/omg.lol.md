---
title: omg.lol
date: 2021-09-26
updated: 
description:  A place to put your now page and your profile page.
tags: weirdweb
---
# omg.lol
[Me](https://home.omg.lol/address/mikehelmers/web)

## Now Page

```markdown
/* This is a sample /now page — edit it and make it your own! */

{profile-picture}

# {address} // you can change this to your name!

## My /now page

--- Now ---

(This is a [now page](https://nownownow.com/about), and if you have your own site, [you should make one](https://nownownow.com/about), too.)

### What I’m reading

- [Wuthering Heights](https://en.wikipedia.org/wiki/Wuthering_Heights) by [Emily Brontë ](https://en.wikipedia.org/wiki/Emily_Brontë) {book}
- [Monocole](https://monocle.com) {book}

### What I’m watching

- [The Jeffersons](https://en.wikipedia.org/wiki/The_Jeffersons) {display}
- [Coronation Street](https://en.wikipedia.org/wiki/Coronation_Street) {display}

### What I’m making

- [Sourdough bread](https://www.feastingathome.com/sourdough-bread/) {bread-slice}
- [Socks](https://www.yarnspirations.com/bernat-family-knit-work-socks/BRK0328-001994M.html?cgid=patterns) {socks}

### What I’m enjoying

- Time with friends and family {people-roof}
- Planning for my upcoming trip {plane}

---

{last-updated}

[Back to my omg.lol page!](https://{address}.omg.lol)
```

## web
```markdown
/*

Hey, welcome to your new omg.lol profile page! With our editor, your profile is a Markdown document. Just edit this example below as you see fit, or read up on the details here: home.omg.lol/info/advanced-editor

*/

{profile-picture}

# Your Name

| Pronouns: (put your pronouns here if you want)
| Occupation: Wanderer
| Location: Earth

--- Bio ---

Say something about yourself here.

--- Profile Items ---

- [Example Link](https://example.com)
- [Another Example](https://example.com)

// You can also add private comments to your profile, like this.

/*
  Have fun editing your new profile!
	(◍•ᴗ•◍)❤
	  - omg.lol
*/


```